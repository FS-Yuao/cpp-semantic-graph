...
struct ChipUpdate : Middle<SoC>, NonCopyable, public Logger {};   // 多继承
}
```

语法解析器（tree-sitter）能看出 `ChipUpdate` 是个 class，后面跟着三个基类说明符的**文本片段**。它**无法**确定：

- `Middle<SoC>` 是特化 `nv::Middle<SoC>`——需要**模板实例化**。
- `Logger` 在哪个命名空间——需要**名字查找**。
- 继承链的根是 `virtual Base`——需要**展开模板并跟随 referenced 声明**。
- `PerformUpdate() override` 到底重写了哪个基类方法——需要**跨类虚函数匹配**。
...
|------|--------------------------|
| "这个类在哪定义的？" | `cpp_search_class("FirmwareUpdate")` → 命名空间 + 文件位置 |
| "谁调用了这个函数？" | `cpp_get_callers("QueryBootChain")` → 所有调用方 |
| "改这个头文件会影响什么？" | 增量更新自动递归 include 依赖，只重解析受影响的 TU |
| "虚函数有哪些 override？" | `cpp_get_overrides("PerformUpdate", "DeviceAdapter")` → 所有实现 |
| "这个模块的架构是怎样的？" | `cpp_traverse_graph("FirmwareUpdate")` → 多跳遍历关联节点 |
...
| # | 工具名 | 用途 | 典型场景 |
|---|--------|------|---------|
| 1 | `cpp_search_class` | 按类名搜索类定义 | "FirmwareUpdate 类在哪定义？" |
| 2 | `cpp_search_function` | 按函数名搜索函数定义 | "PerformUpdate 的签名是什么？" |
| 3 | `cpp_get_inheritance` | 查询类的继承关系 | "DeviceAdapter 有哪些子类？" |
| 4 | `cpp_get_callers` | 查询谁调用了指定函数 | "谁调用了 QueryBootChain？" |
| 5 | `cpp_get_callees` | 查询指定函数调用了谁 | "PerformUpdate 内部调用了什么？" |
| 6 | `cpp_get_overrides` | 查询虚函数的所有重写 | "PerformUpdate 有哪些 override？" |
| 7 | `cpp_get_file_symbols` | 查询文件内的所有符号 | "chip_update.cpp 里有什么？" |
| 8 | `cpp_traverse_graph` | 多跳遍历图谱 | "修改 FirmwareUpdate 会影响什么？" |
| 9 | `cpp_search_docs` | 搜索项目文档（含关联代码） | "OTA 升级流程的设计文档" |
| 10 | `cpp_blast_radius` | 改动爆炸半径（递归调用方 + 虚函数 override 展开 + 文件聚合 + 按跳数分层） | "我要改 PerformUpdate，哪些文件必须 review？" |
| 11 | `cpp_get_code_docs` | 反向：代码符号 -> 描述它的文档 | "哪些设计文档讲到了 PerformUpdate？" |
...
→ ## 搜索结果：类 "FirmwareUpdate"（1 个）
  ### update::FirmwareUpdate
  - 文件: chip_update.h:15-120
```
...
→ ## 搜索结果：函数 "PerformUpdate"（1 个）
  ### FirmwareUpdate::PerformUpdate [virtual, override]
  - 签名: void PerformUpdate() override
  - 文件: chip_update.cpp:45
```
...
→ ## 调用 "QueryBootChain" 的代码（3 个）
  ### UpdateManager::CheckBootChain
  - 文件: update_manager.cpp:128
  - 调用类型: calls_direct
```
...
→ ## 遍历结果：从 "FirmwareUpdate" 出发（18 个节点）
  深度: 2, 遍历边数: 22
  ### 关联节点
