...
**典型场景**：`UpdateManager::UpdateThread` 内有 7 处调用 `HandleError()`（L241/251/568/602/637/689/761），但 `cpp_get_callers("HandleError")` 只返回 1 条 `UpdateThread → HandleError`，无法看到具体在哪些行号调用。
...
UpdateManager::Init          @L211
UpdateManager::UpdateThread  @L533
UpdateManager::HandleError   @L1172
```

**修复后**：9 条边
```
ErrorHandler::HandleError @L17
UpdateManager::HandleError  @L1219
UpdateManager::Init         @L241
UpdateManager::Init         @L251
UpdateManager::UpdateThread @L568
UpdateManager::UpdateThread @L602
UpdateManager::UpdateThread @L637
UpdateManager::UpdateThread @L689
UpdateManager::UpdateThread @L761
```
...
```bash
cd example_update_service/_tools
