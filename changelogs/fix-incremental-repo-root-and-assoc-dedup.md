...
`<WORKSPACE>`(顶层损坏空 `.git`,只有 `info/` 子目录),而非真正的
`workspace` 仓库根。

**根因(双重)**:

- A. `Path(compile_commands).parent.parent` 跳过了 workspace 本身
  - `compile_commands` 在 `workspace/compile_commands.json`,`parent` = workspace(正确起点)
  - 代码用 `parent.parent` = ap,跳过了 workspace
- B. `.git` 存在性检查不验有效性
  - `workspace/.git` 是损坏空目录(只有 `info/`),`.exists()` 返回 True 误判为仓库根
  - workspace/.git 是符号链接 -> `workspace.git`(真 bare 仓库,含 HEAD)

**后果**:在 workspace(非有效仓库)跑 `git diff` 失败,返回 0 变更,
...
| 文件 | 改动 |
|---|---|
| [parser/change_detector.py:190](../parser/change_detector.py#L190) | `_ensure_repo_root`:`parent.parent` -> `parent`(从 workspace 起)+ HEAD 有效性验证(符号链接/file/目录均含 HEAD);去掉无意义的 `for sp` 死循环(循环体未用 `sp`) |
...
| 指标 | 修前 | 修后 |
|---|---|---|
| 推断 repo_root | workspace(❌ 空 .git) | workspace(✅ 真仓库) |
| 检测变更 | 0 | 10(M6+A4) |
| 受影响 TU | 0 | 9 |
