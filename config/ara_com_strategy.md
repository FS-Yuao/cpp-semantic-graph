...
| 路径 | 内容 |
|------|------|
| `ap-model/integration/src-gen/example_update_service/include/` | 生成的头文件（Proxy/Skeleton/类型定义） |
| `ap-model/integration/src-gen/example_update_service/src/` | 生成的源文件（Proxy/Skeleton 实现、序列化/反序列化） |

## 2. 入库策略

| 范围 | 处理方式 | 理由 |
|------|---------|------|
| 生成的头文件（接口声明） | **入库** | 业务代码调用 Proxy 方法（BootChainChanged/QueryBootChain/EnterUpgrade），需要这些接口定义 |
...
只入库以下类型的符号：
- Proxy 类定义及其方法声明（如 `UpdateServiceInterfaceProxy::BootChainChanged`）
- Skeleton 类定义及其方法声明
...
- ✅ Proxy 类识别成功：`UpdateServiceInterfaceProxy`, `DoipServiceInterfaceProxy`
