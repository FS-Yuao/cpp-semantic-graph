...
| `symbol:` | `symbol:` + 原始符号名（保留大小写） | `symbol:SensorUpdate::TryStart` |
...
高置信度（ClassName::Method）：
  ...
  示例: `SensorUpdate::TryStart` → symbol:SensorUpdate::TryStart

中置信度（单独 ClassName）：
  正则: `([A-Z][A-Za-z0-9_]{2,})`
  过滤: 黑名单（OTA/UDS/HTTP...）+ 全大写缩写 + kXxx 枚举值
  示例: `UpdateManager` → symbol:UpdateManager
...
**桥接效果**：用户问「TryStart 相关的文档和代码」，一次查询返回：

```
文档侧：
  diary-2026-07-15: "TryStart 有激活 bug"（根因分析）
  task/sensor-version-check-fix: "修复 TryStart 阶段版本校验缺陷"（已完成）

代码侧（通过符号桥接从 cppsg 查到）：
  function: update::PeriAdapter::TryStart
    定义: device_manager/device_adapter.cpp
    调用方: UpdateManager::StartUpdate
    被调用: SensorUpdate::CompareVersion
```
