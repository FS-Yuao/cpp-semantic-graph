...
| **P1** | 符号边重复爆炸 | task/sensor_activate_version_check_fix | 同 src+dst+rel 去重 |
...
| 5 | TryStart 相关的文档和代码 | diary + task + cppsg 节点 | 符号桥接 |
| 6 | GNSS 版本校验怎么做的 | sensor task + diary | FTS5 + BFS |
...
| **前置依赖** | Task 6 |
| **产出物** | MCP 工具配置（`~/.codebuddy/mcp.json` 新增条目）<br>`doc_graph/mcp_server.py`（MCP 服务端）<br>CLAUDE.md 搜索决策树更新 |
