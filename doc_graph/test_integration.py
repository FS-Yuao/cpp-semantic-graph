...
def test_fts5(conn):
    ...
    keywords = ["分区", "OTA", "GNSS", "分区切换", "UpdateManager"]
    for kw in keywords:
...
def test_code_doc_bridge(conn):
    ...
    known_funcs = ["doPartitionSwitch", "PerformUpdate", "UpdateSinglePeripheral",
...
def test_reverse_lookup(conn):
    ...
    test_cases = [
        ...
        ("PerformUpdate", ["OTA", "Update", "upgrade"]),
        ("UpdateSinglePeripheral", ["Update", "peri", "upgrade"]),
        ("UpdateManager", ["OTA", "manager", "state"]),
    ]
