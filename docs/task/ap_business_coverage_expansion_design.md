# 设计文档：cppsg 覆盖范围扩展至 workspace 全业务模块 + doc_query 多关键词 + clangd 优先搜索规则

> 版本：v1 | 日期：2026-07-15 | 关联：README FAQ「条件编译盲区」、近两周 MCP 使用情况分析

## 1. 目标

1. **扩展代码覆盖范围**：把 cpp_semantic_graph（cppsg）的解析范围从单一 `example_update_service` 扩展到 workspace/app 下全部 8 个 `hq_*` 业务模块 + `util` 公共库（共 141 个 TU），构建覆盖整个 AP 业务层的语义图谱。
...
```yaml
source_paths:
  - "example_update_service/src"
  - "example_update_service/include"
```

compile_commands.json 共 669 个 TU，cppsg 仅覆盖 example_update_service 的 32 个（4.8%）。后果：
...
| workspace `CLAUDE.md` | 修改 | 搜索决策树补 clangd 优先降级规则 |
| workspace `memory/` | 新增 | clangd 优先搜索规则详细版（feedback 类） |
| `README.md` / `README_zh.md` | 同步 | 覆盖范围说明从"example_update_service"改为"workspace 业务模块"（如有相关措辞） |

> 注：`docs_dir` 仍指向 example_update_service/docs（单路径字符串，扩多目录需改 config.py+doc_ingester+change_detector 三文件，超出"包含代码"诉求，列为后续工作，见风险点 R3）。
...
|---|---|---|---|
| example_update_service | `example_update_service/src`、`example_update_service/include` | 32 | 现有，保留 include |
...
**风险一：裸 `"util"` 过宽**。会误匹配 `example_update_service/src/device_utils`、`hq_diag_did_rid_app/src/utils`，且调用边过滤 `should_extract_call` 会把任何 callee 路径含 "util" 的 SDK 头误判为项目代码，污染图谱。故枚举三个子目录而非裸 `"util"`。
...
`project.name` 仅用于 `mcp_server/server.py:64` 系统提示词展示标签（"适用于 {name} 项目的 C++ 代码查询场景"），无路径/过滤功能。从 `example_update_service` 改为 `workspace 业务模块`，使 MCP 自描述与新覆盖范围一致。
...
写入 workspace `CLAUDE.md` 搜索决策树末尾 + memory（feedback 类）：
...
- [ ] workspace CLAUDE.md 搜索决策树含 clangd 优先降级规则。
