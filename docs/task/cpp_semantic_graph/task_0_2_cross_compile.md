...
2. **验证 libclang 传参方式**：
   ...
   tu = index.parse("chip_update.cpp", args=[
       "-std=c++17",
...
- [ ] libclang 能在交叉编译环境下成功解析核心文件（`chip_update.cpp`、`base_device_update.h`）
- [ ] 解析结果与 clangd `get_type_hierarchy` 结果一致（如 DeviceAdapter 的 4 个子类）
