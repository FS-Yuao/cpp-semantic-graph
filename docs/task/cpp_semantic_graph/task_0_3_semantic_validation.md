...
|--------|---------|-------------------|---------|
| 类定义与命名空间 | 提取 DeviceAdapter 及其 4 个子类 | `get_type_hierarchy` | 结果一致 |
| 继承关系（含权限） | 提取 FirmwareUpdate → DeviceAdapter 的 public 继承 | `get_type_hierarchy` | 权限、虚继承标记正确 |
...
class SemanticValidator:
    ...
    def validate_class_hierarchy(self):
        """验证类与继承关系"""
        # 提取 DeviceAdapter 的子类，对比 clangd 结果
