...
1. 选取核心模块（chip_update.cpp），手动标注所有函数调用点
...
1. 在 chip_update.cpp 中手动标注所有函数调用点（ground truth）
...
- MEMBER_REF_EXPR.referenced 可正确解析 Proxy 方法调用（BootChainChanged/QueryBootChain/EnterUpgrade）
