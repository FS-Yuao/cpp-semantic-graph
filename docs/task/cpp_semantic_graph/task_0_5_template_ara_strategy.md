...
白名单条目的筛选标准：
- 项目代码中有 `using` / `typedef` 别名的模板特化（如 `using UpdateServiceProxy = Proxy<...>`）
...
- 模板白名单已创建：UpdateServiceInterfaceProxy、DoipServiceInterfaceProxy、PowerStatusServiceInterfaceProxy
