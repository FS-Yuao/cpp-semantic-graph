...
```bash
# 按类名搜索
python -m cpp_semantic_graph search-class "FirmwareUpdate"

# 查继承关系（向下 2 级）
python -m cpp_semantic_graph inheritance "DeviceAdapter" --direction down --depth 2

# 按函数名搜索
python -m cpp_semantic_graph search-func "PerformUpdate"

# 查文件内符号
python -m cpp_semantic_graph file-symbols "chip_update.cpp"
```
...
- [x] `search_class("DeviceAdapter")` 返回正确，含命名空间（`update::`）和文件路径
- [x] `get_inheritance("DeviceAdapter", "down")` 返回 4 个子类（FirmwareUpdate, SensorUpdate, SwitchUpdate, McuUpdate）
- [x] `search_function("PerformUpdate")` 返回函数签名、所属类、是否 override（9 条，基类纯虚 / 子类 override）
- [x] `get_file_symbols("chip_update.cpp")` 返回文件内所有类和函数（29 符号）
...
- **端到端验证通过**（5 个核心模块 cpp，0 fatal error）：
  ...
  - 继承边正好 4 条，与设计预期一致
- **查询验证**（CLI + API 双跑）：
  - `search_class("DeviceAdapter")` → 1 个，`update::DeviceAdapter` (abstract)，base_device_update.h:13-80
  - `get_inheritance("DeviceAdapter", "down")` → 4 个子类，全 `--public-->`
  - `search_function("PerformUpdate")` → 9 个，基类 `pure=True/override=False`、子类 `override=True` 正确区分
  - `get_file_symbols("chip_update.cpp")` → 29 符号，含 FirmwareUpdate 全部成员函数
...
2. **search_function 返回声明+定义**：PerformUpdate 返回 9 条（.h 声明 + .cpp 定义各一）。这是 task 1-2 的设计——unique_key 含 file_path 区分声明与定义。是否符合预期待 task 1-4 准确性验证确认
