...
| `tools/cpp_semantic_graph/validation/test_query_api.py` | 修改，CORE_FILTERS 增加 device_manager（调用关系验证需覆盖调用方文件） |
...
validation_samples:
  classes:
    - name: "DeviceAdapter"
      expected_children: 4    # FirmwareUpdate, SensorUpdate, SwitchUpdate, McuUpdate
    - name: "FirmwareUpdate"
      expected_parent: "DeviceAdapter"
    - name: "UpdateServiceProxy"  # 模板实例化
  functions:
    - name: "PerformUpdate"
...
- [x] 核心模块（example_update_service）验证结果达标：Precision ≥ 95%，Recall ≥ 90%（实测 4 维度均 100%/100%）
- [x] 验证报告包含逐条对比详情（哪些一致、哪些不一致、原因分析）
- [x] 验证样本集覆盖核心类（DeviceAdapter 及 4 子类）和核心函数（PerformUpdate / GetPeriName / ExecuteDriveUpdate）
...
- **验证样本**：2 类（DeviceAdapter + FirmwareUpdate）、6 函数（PerformUpdate ×5 + GetPeriName ×1）、2 调用引用（PerformUpdate + ExecuteDriveUpdate）
...
- **关键设计决策**：
  ...
  2. **调用关系 baseline 取静态语义** — 图谱阶段 1 只做静态调用（C++ 标准语义），`agent->impl->PerformUpdate()` 的边指向静态类型 `DeviceAdapter::PerformUpdate`（calls_virtual）。baseline 据此以基类为 owner，不取 clangd find_references 的多态动态解析结果。动态多态解析是 task 2-1 的明确目标
  3. **路径归一化** — 图谱存相对路径、baseline 存带前缀路径，对比时取末两段路径段归一化
- **调试过程发现的 3 个问题及处理**：
  1. 调用关系初版 0%/0% → 发现是验证样本未含 device_adapter.cpp（调用方文件未入库）→ test_query_api.py 增 device_manager，Recall 升至 50%
