...
| `tools/cpp_semantic_graph/validation/clangd_baseline.json` | 修改，补充 update_manager.cpp 调用方 |
...
- [x] 目标项目（example_update_service）全量解析成功（25 TU，0 失败）。hq_vehicle_service 同机制可用，未单独跑（共用 compile_commands）
...
- **全量解析 example_update_service**（25 翻译单元，串行）：
...
- **include 查询验证**（3 模式）：
  - `include base_device_update.h -m direct` → 6 个 TU（4 update + device_adapter + update_factory）
  - `include chip_update.h -m all` → 2 个受影响 TU（直接 + 间接）
...
- 调用关系维度初跑 Precision 66.7%（FP 1 个：`update_manager.cpp`）。核查发现 `update_manager.cpp:879` 的 `updater->PerformUpdate()`（虚调用）**真实存在**，图谱正确建边，但 clangd `find_references` 未返回该调用点（4 处/10 处均不含）。故属 baseline 漏采，非图谱错误
- 修正 baseline.json 补充 update_manager.cpp 调用方后，调用关系回归 100%/100%
...
2. **hq_vehicle_service 未单独全量跑**：与 example_update_service 共用 compile_commands.json 和同一套配置机制，可随时切换 filter 跑，未在本 task 单独执行
