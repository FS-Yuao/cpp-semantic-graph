...
- [ ] `get_full_inheritance_chain("DeviceAdapter", "down")` 返回完整子类树，无遗漏
- [ ] `get_virtual_functions("DeviceAdapter")` 返回所有虚函数，含继承来的
- [ ] `get_all_overrides("DoUpdate", "DeviceAdapter")` 返回所有子类的重写实现
- [ ] `get_all_implementations("DeviceAdapter")` 只返回非抽象的叶子子类
