...
- 当前图谱没有调用关系数据，无法回答"谁调用了 QueryBootChain"这类问题
- 调用关系是架构分析和影响面分析的基础
...
- 虚调用只能看到基类声明，**通过 override 关系补全所有可能的目标函数**
  - 例如：`basePtr->DoUpdate()` → 调用目标是 `FirmwareUpdate::DoUpdate`、`SensorUpdate::DoUpdate`、`SwitchUpdate::DoUpdate`、`McuUpdate::DoUpdate`
...
- [ ] `get_callers("QueryBootChain")` 返回正确的调用方列表
- [ ] `get_callees("PerformUpdate", "FirmwareUpdate")` 返回 PerformUpdate 内部的调用链
