...
class ArchitectureQuery:
    def get_interface_implementations(self, interface_name: str) -> list[ClassInfo]:
        ...
        - 示例：get_interface_implementations("DeviceAdapter")
          → [FirmwareUpdate, SensorUpdate, SwitchUpdate, McuUpdate]
        """
    ...
    def get_virtual_function_overrides(self, func_name: str,
        ...
        - 示例：get_virtual_function_overrides("DoUpdate", "DeviceAdapter")
          → [FirmwareUpdate::DoUpdate, SensorUpdate::DoUpdate, ...]
        """
...
- [ ] `get_interface_implementations("DeviceAdapter")` 返回 4 个非抽象子类
- [ ] `get_virtual_function_overrides("DoUpdate", "DeviceAdapter")` 返回所有子类重写，无遗漏
- [ ] `get_class_virtual_table("FirmwareUpdate", include_inherited=True)` 返回含继承来的虚函数
