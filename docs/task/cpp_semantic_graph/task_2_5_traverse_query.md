...
- 典型需求："QueryBootChain 被哪些子类的 DoUpdate 调用了？"需要先查调用者再过滤继承关系
- 典型需求："修改 DeviceAdapter 会影响哪些模块？"需要沿继承+调用+文档多跳展开
...
def traverse_graph(
    ...
    示例 1：QueryBootChain 被哪些子类的 DoUpdate 调用了？
    traverse_graph(
        start="QueryBootChain",
        relation_types=["calls_direct", "calls_virtual"],
        direction="incoming",     # 反向：谁调用了 QueryBootChain
        depth=1,
        filters={"function_name": "DoUpdate"}  # 只保留 DoUpdate
    )

    示例 2：修改 DeviceAdapter 会影响哪些模块？
    traverse_graph(
        start="DeviceAdapter",
...
- [ ] 示例查询结果正确："QueryBootChain 的调用者中哪些是 DeviceAdapter 子类的 DoUpdate"
