...
```markdown
## FirmwareUpdate 升级流程设计

FirmwareUpdate 继承 [[DeviceAdapter]]，实现 SoC 固件升级。
升级流程调用 [[PerformUpdate]] 方法，内部使用 [[ExecuteDriveUpdate]] 执行 driveupdate 工具。
```
...
class DocAssociationParser:
    def parse_manual_links(self, doc_section: DocSection) -> list[Association]:
        ...
        - 支持限定命名空间：[[example_ns::FirmwareUpdate]]
...
1. **标记语法歧义**：`[[Update]]` 可能匹配多个类（FirmwareUpdate、SensorUpdate...），需支持命名空间限定
