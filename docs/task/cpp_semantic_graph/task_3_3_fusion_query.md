...
- AI 查 `FirmwareUpdate` 时应该同时返回"升级流程设计"的文档片段
- AI 查"升级流程设计"文档时应该同时返回 FirmwareUpdate 类和关键函数
...
traverse_graph(
    start="FirmwareUpdate 升级流程设计",          # 文档节点
...
```json
{
  "class": {
    ...
    "inheritance": {"parent": "DeviceAdapter", "children": []}
  },
  "related_docs": [
    {
      "title": "FirmwareUpdate 升级流程设计",
      "doc_path": "docs/OTA_flow/soc_upgrade_design.md",
      "content_preview": "FirmwareUpdate 继承 DeviceAdapter...",
      "confidence": 1.0,
      "method": "manual"
    }
  ]
...
- [ ] `search_class_with_docs("FirmwareUpdate")` 返回类信息 + 关联的架构设计文档片段
- [ ] `search_documentation("升级流程")` 返回文档切片 + 关联的代码实体
- [ ] `get_docs_for_class("DeviceAdapter")` 返回所有关联文档
- [ ] 文档查询支持按标签过滤（tag 参数）
