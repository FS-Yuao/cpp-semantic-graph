...
| 检查项 | 验证方法 | 通过标准 |
|--------|---------|---------|
| C++ 类搜索 | `cpp_search_class("FirmwareUpdate")` | 返回正确，含命名空间和文件路径 |
| 继承关系 | `cpp_get_inheritance("DeviceAdapter", "down")` | 返回 4 个子类 |
| 调用关系 | `cpp_get_callers("QueryBootChain")` | 返回调用方 |
| 文档关联 | `cpp_search_class("FirmwareUpdate", with_docs=true)` | 返回关联文档 |
| 多跳遍历 | `cpp_traverse_graph("DeviceAdapter", [...], depth=3)` | 返回完整影响面 |
| 查询性能 | P95 延迟 | < 50ms |
