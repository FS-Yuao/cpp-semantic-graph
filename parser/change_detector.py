...
example_update_service 所在的 workspace 是独立 git 仓库，需从 source_paths 向上查找 .git。
"""
...
class ChangeDetector:
    ...
    def _ensure_repo_root(self) -> Path | None:
        ...
        均含 HEAD），以此排除损坏的空 .git 目录（如顶层 workspace/.git），
        避免误判为仓库根导致 git diff 失败、增量检测返回空。

        注：compile_commands 通常位于仓库根（workspace/compile_commands.json），
    ...
    def _run_git_diff(self, repo_root: str, base_ref: str) -> list[tuple[str, str]]:
        ...
        git diff --name-status 输出格式:
          M\tapp/example_update_service/src/chip_update.cpp
          A\tapp/example_update_service/include/new_header.h
          D\tapp/example_update_service/src/old_file.cpp
