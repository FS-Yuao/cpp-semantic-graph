...
class FullParsePipeline:
    ...
    def _run_impl(self, db_path: str, *,
        ...
        try:
            ...
            manual_stats = assoc_ingester.ingest_manual_associations()
            assoc_stats.update(manual_stats)
            # 接入标题/标签规则匹配关联（设计文档第 3 层策略，原为死代码）
            rule_stats = assoc_ingester.ingest_rule_associations()
            assoc_stats.update(rule_stats)
