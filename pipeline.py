...
用法:
  ...
  report = pipeline.run(db_path, filter_path="example_update_service")
"""
...
class FullParsePipeline:
    ...
    def run(self, db_path: str, *,
        ...
        Args:
            db_path: 输出数据库路径
            filter_path: 只解析路径含此串的翻译单元（如 "example_update_service"）
        ...
        try:
            ...
            aschip_ingester.close()
            report.doc_associations_new = aschip_stats.get("edges_created", 0)
