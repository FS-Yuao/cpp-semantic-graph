...
```
问题: "PerformUpdate 有哪些 override？"
  clangd find_implementations  → 0 (跨 TU 限制)
  图谱 get_overrides           → 4 (SensorUpdate, McuUpdate, FirmwareUpdate, SwitchUpdate)
```
...
| 场景 | 状态 | 影响 | 根因 |
|------|------|------|------|
| 模板实例化 `vector<FirmwareUpdate>` | ⏸️ 未启用 | 无法追踪模板关联 | libclang 不产生特化 CLASS_DECL |
...
| 难点 | 风险 | 缓解 |
|------|------|------|
| 名字映射: "QueryBootChain" → `getActiveBootChain` | 高 | MCP instructions 加 few-shot |
| 概念→代码: "升级流程" → `PerformUpdate` | 高 | 同上 |
