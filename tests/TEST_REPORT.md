...
> **项目**: example_update_service — automotive OTA service (example project)  
> **测试日期**: 2025-06-25  
...
| 项目 | 值 |
|------|-----|
| 目标代码 | example_update_service (C++17, ARA COM + DUCC) |
...
| 解析范围 | 29 翻译单元 (example_update_service only) |
...
| # | 工具 | 测试用例 | 预期 | 实际 | 判定 |
|---|------|---------|------|------|------|
| 1 | `cpp_search_class` | 精确"FirmwareUpdate" | 1个类定义 | `update::FirmwareUpdate` chip_update.h:9-61 | ✅ |
| 1b | `cpp_search_class` | 模糊"Update" | 多个含 Update 的类 | 10个(DeviceAdapter,SensorUpdate,McuUpdate,FirmwareUpdate,SwitchUpdate,…) | ✅ |
| 2 | `cpp_search_function` | "PerformUpdate" class="FirmwareUpdate" | 声明+定义 | chip_update.h:18 + chip_update.cpp:613 | ✅ |
| 2b | `cpp_search_function` | "PerformUpdate" 全部 | 所有同名函数 | 11个(4子类×2 + DeviceAdapter + PeriAdapter×2) | ✅ |
| 3 | `cpp_get_inheritance` | DeviceAdapter down=-1 | 4个子类 | SensorUpdate, McuUpdate, FirmwareUpdate, SwitchUpdate | ✅ |
| 4 | `cpp_get_callers` | FileExists (ns=FileHandler) | 15个调用函数 | 15个(sensor/mcu/soc/switch + file_handler) | ✅ |
| 5 | `cpp_get_callees` | PerformUpdate (FirmwareUpdate) | 9个被调函数 | Logger::Info/Error, ExecuteDriveUpdate, StartContentServer, … | ✅ |
| 6 | `cpp_get_overrides` | PerformUpdate (DeviceAdapter) | 4个重写类 | SensorUpdate, McuUpdate, FirmwareUpdate, SwitchUpdate (8边去重后4) | ✅ |
| 7 | `cpp_get_file_symbols` | update_manager.cpp | ~50+符号 | 52个function (clangd 61含namespace/variable) | ✅ |
| 8 | `cpp_traverse_graph` | FirmwareUpdate depth=2 both | 多文件多节点 | 52节点/70边/6文件 | ✅ |
...
#### T3 继承关系 — DeviceAdapter 子类

```
update::SensorUpdate  (sensor_update.h)
update::McuUpdate   (mcu_update.h)
update::FirmwareUpdate   (chip_update.h)
update::SwitchUpdate(switch_update.h)
```

与 CLAUDE.md 中的约束对照："不要在 DeviceAdapter 加只有单个子类用的回调"——图谱清晰展示 4 个子类,为架构决策提供数据依据。

#### T5 调用链 — FirmwareUpdate::PerformUpdate

```
FirmwareUpdate::PerformUpdate 调用了:
  ├─ Logger::GetInstance        (日志获取)
  ├─ Logger::Info / Logger::Error (日志输出)
  ├─ DeviceAdapter::GetPeriName  (获取外设名)
  ├─ DeviceAdapter::NotifyProgress (进度通知)
  ├─ FirmwareUpdate::ExecuteDriveUpdate (DRIVE 升级执行)
  ├─ FirmwareUpdate::StartContentServer (内容服务器启动)
  ├─ FirmwareUpdate::StopContentServer  (内容服务器停止)
  └─ IsDirectoryPath               (目录判断)
```

9 条被调用函数完整反映了 FirmwareUpdate 固件升级的内部流程。

#### T8 多跳遍历 — FirmwareUpdate depth=2

从 `FirmwareUpdate` 出发 depth=2 双向遍历,覆盖 6 个文件、52 个节点、70 条边：

| 文件 | 节点数 | 代表符号 |
|------|--------|---------|
| chip_update.h | 22 | FirmwareUpdate, PerformUpdate, TryPrepare, … |
| base_device_update.h | 20 | DeviceAdapter, NotifyProgress, GetPeriName, … |
| chip_update.cpp | 7 | ExecuteDriveUpdate, StartContentServer, … |
| sensor_update.h | 1 | SensorUpdate (兄弟类可见) |
...
3. **override 双计**: 每个虚函数重写产生 2 条边(.h 声明 + .cpp 定义),如 `PerformUpdate` 8 边去重后 4 个实际重写类。不影响精度(去重即可),但计数显示翻倍。
...
|---|------|---------|---------|------------|------|------|------|
| 1 | 继承(子类) | DeviceAdapter↓ | 4 子类 | 4 子类 | 100% | 100% | 完全一致 |
| 2 | 继承(父类) | FirmwareUpdate↑ | DeviceAdapter | DeviceAdapter | 100% | 100% | 完全一致 |
| 3 | Callers | FileHandler::FileExists | 15 函数 | 15 调用点 | ~100% | ~100% | 图谱按函数计数,clangd 按调用点;调用方集合一致 |
| 4 | Overrides | DeviceAdapter::PerformUpdate | **4** 重写 | **0** | — | **图谱>clangd** | clangd 对跨文件虚分派返回空(已知限制) |
| 5 | FileSymbols | update_manager.cpp | 52 函数 | ~53 函数 | 98% | 98% | 差1(重载合并),图谱不含 namespace/variable |
| 6 | Overrides | DeviceAdapter::TryPrepare | 4 重写 | 0 | — | **图谱>clangd** | 同上,clangd 无法跨 TU 解析 override |
| 7 | Overrides | DeviceAdapter::Cancel | 4 重写 | 0 | — | **图谱>clangd** | 同上 |
...
- 三档搜索范围: **小**(example_update_service src+include) / **中**(workspace/app) / **大**(workspace+model+foundation)
...
| PerformUpdate | **0.23** | 3.09 | 189.76 | 247.84 | 222.89 |
...
3. **大规模下图谱快 1000×**：在跨项目(workspace+model+foundation)搜索中,图谱 1ms vs grep 1s
...
### 6.2 交叉编译 target 缺失 → update_manager.cpp 解析成功

| 指标 | 修复前 | 修复后 |
|------|--------|--------|
| update_manager.cpp 状态 | failed (fatal error) | **success** |
| update_manager.cpp 节点 | 0 | **52** |
...
| 范围 | 总 TU | 成功 | 失败 | 成功率 |
|------|-------|------|------|--------|
| example_update_service | 29 | 29 | 0 | **100%** |
...
### 场景 1: "DeviceAdapter 有哪些子类？"

```
图谱: cpp_get_inheritance("DeviceAdapter", direction="down", depth=-1)
结果: SensorUpdate, McuUpdate, FirmwareUpdate, SwitchUpdate
clangd 验证: 完全一致 ✅
grep 等价: grep -rn "class.*: public.*DeviceAdapter" --include='*.h' → 需人工过滤,耗时 200ms+
```

**图谱优势**: 0.3ms 返回结构化结果;grep 需人工解读正则匹配,且漏 protected/private 继承

### 场景 2: "谁调用了 QueryBootChain？"（CLAUDE.md 关键约束函数）

```
图谱: cpp_get_callers("getActiveBootChain") → UpdateManager, UpdateServiceInterface, MccAdapter 等
...
### 场景 4: "改 base_device_update.h 会影响什么？"（增量更新场景）

```
图谱: cpp_traverse_graph("DeviceAdapter", depth=2)
...
### 场景 5: "PerformUpdate 有哪些 override？"（虚分派分析）

```
图谱: cpp_get_overrides("PerformUpdate", "DeviceAdapter") → 4 个重写类
clangd: find_implementations("PerformUpdate") → 0 (跨 TU 限制)
```
...
结果: 3 个文档切片:
  - AB_PARTITION_SWITCH_DESIGN.md "分区切换机制" (标签: 架构设计, A/B分区)
    关联代码: BootChainChanged(conf=0.70), FirmwareUpdate(0.70), TryStart(0.60), UpdateService(0.70)
  - task_2_2_call_relation.md "现状问题" (标签: 开发任务)
    关联代码: QueryBootChain(0.70)
  - task_2_5_traverse_query.md "现状问题" (标签: 开发任务)
    关联代码: QueryBootChain(0.70), DeviceAdapter(0.70)
...
|---|--------|------|-----------|------|---------|--------|------|
| 1 | dry-run (.cpp) | `chip_update.cpp --dry-run` | 1 | <1s | 不变 | — | ✅ |
| 2 | dry-run (.h) | `chip_update.h --dry-run` | 2 | <1s | 不变 | — | ✅ |
| 3 | 实际更新 (.cpp) | `chip_update.cpp` | 1 | 11.3s | 1631n/3211e | — | ✅ |
| 3b| 幂等性 (.cpp) | `chip_update.cpp` 二次 | 1 | 11.6s | 1631n/3211e | ✅ 边数稳定 | ✅ |
| 4 | 实际更新 (.h) | `base_device_update.h` | 7 | 76.4s | 1631n/3010e | — | ✅ |
| 4b| 幂等性 (.h) | `base_device_update.h` 二次 | 7 | 72.6s | 1631n/3010e | ✅ 边数稳定 | ✅ |
| 5 | git diff 模式 | `--base HEAD` | — | — | — | — | ⚠️ |

### 8.2 .h 文件影响链

修改 `chip_update.h` 的 dry-run 输出：

```
影响链:
  device_update/soc/chip_update.h → 2 个 TU
    - device_manager/update_factory.cpp
    - device_update/soc/chip_update.cpp
```

修改 `base_device_update.h` 触发 **7 个 TU** 重解析（所有 4 个子类的 .cpp + 相关管理器），体现了 include 依赖图的递归分析能力。

### 8.3 幂等性验证

| 测试 | 第1次边数 | 第2次边数 | 差异 | 判定 |
|------|----------|----------|------|------|
| chip_update.cpp | 3211 | 3211 | 0 | ✅ 完全幂等 |
| base_device_update.h | 3010 | 3010 | 0 | ✅ 完全幂等 |
...
| 项目 | 结果 | 原因 |
|------|------|------|
| repo 工具仓库 | ⚠️ 失败 | `_ensure_repo_root` 找到 `<WORKSPACE>/.git`（repo 顶层）而非 `workspace/.git`（符号链接到 .repo/projects） |
...
| 3 | 模板实例化边未启用 | 无法追踪 `vector<FirmwareUpdate>` 等模板关联 | P2 | libclang 限制,TemplateExtractor 保留待启用 |
...
- 交叉编译 target 缺失 → update_manager.cpp 52 节点入库 ✅
- 全量解析成功率 100%(example_update_service 范围) ✅
