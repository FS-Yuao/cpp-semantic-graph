...
SRC = "<PROJECT_SRC>/src"
INC = "<PROJECT_SRC>/include"
SCOPE_SMALL = [SRC, INC]
SCOPE_MID = ["<PROJECT_APP>"]
SCOPE_LARGE = ["<WORKSPACE>",
               "<WORKSPACE>/drive-vendor/ap-model",
               "<WORKSPACE>/drive-vendor/ap-foundation"]
...
def tool_search_class():
    print("\n[A1] cpp_search_class")
    print("  用例1 精确: 'FirmwareUpdate'")
    rows = conn.execute(
        "SELECT name, namespace, file_path, start_line, end_line FROM node "
        "WHERE type='class' AND name='FirmwareUpdate'").fetchall()
...
def tool_search_function():
    ...
    print("  用例: 'PerformUpdate' class_name='FirmwareUpdate'")
    rows = conn.execute(
        "SELECT DISTINCT name, namespace, file_path, start_line FROM node "
        "WHERE name='PerformUpdate' AND type='function' "
        "AND namespace='update::FirmwareUpdate' ORDER BY file_path").fetchall()
    for r in rows: print(f"    -> {r[1]}::{r[0]}  {r[2]}:{r[3]}")
    all_pf = conn.execute(
        "SELECT DISTINCT name, namespace, file_path FROM node WHERE name='PerformUpdate' "
        "AND type='function' ORDER BY namespace, file_path").fetchall()
    print(f"  全部同名 PerformUpdate: {len(all_pf)} 个（含各子类 + .h声明/.cpp定义）")
...
def tool_get_inheritance():
    ...
    print("  用例: DeviceAdapter direction=down depth=-1")
    rows = conn.execute(
        "SELECT n.name, n.namespace, n.file_path FROM node n "
        "JOIN edge e ON e.from_id=n.id JOIN node p ON p.id=e.to_id "
        "WHERE p.name='DeviceAdapter' AND e.relation_type='inherits_public' "
...
def tool_get_overrides():
    ...
    print("  用例: 'PerformUpdate' class_name='DeviceAdapter'")
    rows = conn.execute(
        ...
        "WHERE e.relation_type='overrides' AND tn.name='PerformUpdate' "
        "AND tn.namespace='update::DeviceAdapter' ORDER BY fn.namespace").fetchall()
...
def tool_get_file_symbols():
    ...
    print("  用例: 'update_manager.cpp'")
    rows = conn.execute(
        "SELECT name, type, namespace, start_line FROM node "
        "WHERE file_path='update_manager/update_manager.cpp' ORDER BY start_line").fetchall()
...
def tool_traverse_graph():
    ...
    print("  用例: start='FirmwareUpdate' depth=2 direction=both")
    start = conn.execute(
        "SELECT id FROM node WHERE name='FirmwareUpdate' AND type='class'").fetchone()
    if not start:
...
def efficiency():
    ...
    cases = [
        ...
        ("PerformUpdate",  r"\bPerformUpdate\s*\("),
    ]
...
def bug_fixes():
    ...
    print()
    print("D2. 交叉编译 target 缺失 → update_manager.cpp 解析成功")
    n = conn.execute(
        "SELECT COUNT(*) FROM node WHERE file_path='update_manager/update_manager.cpp'").fetchone()[0]
    ps = conn.execute(
        "SELECT status FROM parse_status WHERE source_file LIKE '%update_manager.cpp'").fetchall()
    print(f"    update_manager.cpp: 节点={n}, parse_status={[r[0] for r in ps]}")
    ...
    rows = conn.execute(
        ...
        "AND tn.namespace='update::DeviceAdapter'").fetchall()
    ...
    print(f"    (example_update_service 范围: 29 TU 全成功，0 失败) {'✓' if failed==0 else '✗'}")
