...
def test_functional():
    ...
    with BlastRadiusQuery(DB) as bq:
        ...
        r = bq.compute(symbols=["PerformUpdate"], depth=3, direction="up")
        ...
        check("A2 override 含子类",
              {"FirmwareUpdate", "SensorUpdate", "McuUpdate", "SwitchUpdate"} & override_classes,
              f"{override_classes}")

        # A3: 文件输入 → 展开为符号集
        r_file = bq.compute(files=["base_device_update.cpp"], depth=2, direction="up")
        check("A3 文件输入展开为符号",
        ...
        r_down = bq.compute(symbols=["PerformUpdate"], depth=2, direction="down")
        check("A5 direction=down 返回结果",
              len(r_down.affected_nodes) >= 0)  # 至少不报错

        # A6: include_overrides=False 不展开 override
        r_noov = bq.compute(symbols=["PerformUpdate"], depth=2,
...
def test_system_regression():
    ...
    with BlastRadiusQuery(DB) as bq:
        bq.compute(symbols=["PerformUpdate"], depth=3)
        bq.compute(files=["base_device_update.cpp"], depth=2)
    ...
    with BlastRadiusQuery(DB) as bq:
        r = bq.compute(symbols=["PerformUpdate"], depth=2,
                       include_overrides=True, direction="up")
    with PolymorphismQuery(DB) as pq:
        # PerformUpdate 基类是 DeviceAdapter
        ovs = pq.get_all_overrides("PerformUpdate", "DeviceAdapter")
    ...
    with BlastRadiusQuery(DB) as bq:
        r1 = bq.compute(symbols=["PerformUpdate"], depth=1,
                        include_overrides=False, direction="up")
    with CallQuery(DB, expand_virtual=False) as cq:
        # 取 DeviceAdapter::PerformUpdate 的直接 callers
        callers = cq.get_callers("PerformUpdate", class_name="DeviceAdapter")
...
def test_falsification():
    ...
    with BlastRadiusQuery(DB) as bq:
        ...
        r_d1 = bq.compute(symbols=["PerformUpdate"], depth=1, direction="up")
