...
def test_reverse():
    ...
    with DocQuery(DB) as dq:
        # A1: PerformUpdate 返回设计文档
        docs = dq.get_docs_for_function("PerformUpdate")
        check("A1 PerformUpdate 返回关联文档",
        ...
        # A3: FirmwareUpdate（类）反向也工作
        docs_c = dq.get_docs_for_class("FirmwareUpdate")
        check("A3 FirmwareUpdate 类反向返回文档",
...
def test_system_regression():
    ...
    with DocQuery(DB) as dq:
        dq.get_docs_for_function("PerformUpdate")
        dq.get_docs_for_class("FirmwareUpdate")
...
def test_falsification():
    section("D. 证伪测试")
    with DocQuery(DB) as dq:
        # D1: min_confidence=1.0 仅高质量（PerformUpdate 的关联都是0.6，应返回空或极少）
        docs_10 = dq.get_docs_for_function("PerformUpdate", min_confidence=1.0)
        docs_00 = dq.get_docs_for_function("PerformUpdate", min_confidence=0.0)
