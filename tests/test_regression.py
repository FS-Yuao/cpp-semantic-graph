...
用法:
    cd <WORKSPACE> && PYTHONPATH=. python3 cpp_semantic_graph/tests/test_regression.py

也可用 pytest:
    cd <WORKSPACE> && PYTHONPATH=. python3 -m pytest cpp_semantic_graph/tests/test_regression.py -v
...
def test_search_apis():
    ...
    with GraphQuery(DB_PATH) as q:
        # search_class
        classes = q.search_class("FirmwareUpdate", exact=True)
        check(len(classes) >= 1, f"search_class('FirmwareUpdate') → {len(classes)} results")

        # search_function
        funcs = q.search_function("PerformUpdate")
        check(len(funcs) >= 1, f"search_function('PerformUpdate') → {len(funcs)} results")

        # get_file_symbols
        symbols = q.get_file_symbols("chip_update.cpp")
        check(len(symbols) >= 1, f"get_file_symbols('chip_update.cpp') → {len(symbols)} symbols")
...
def test_inheritance():
    ...
    with GraphQuery(DB_PATH) as q:
        # down: 子类
        children = q.get_inheritance("DeviceAdapter", direction="down", depth=1)
        check(len(children) >= 1,
              f"get_inheritance('DeviceAdapter', down) → {len(children)} children")

        # up: 父类
        parents = q.get_inheritance("FirmwareUpdate", direction="up", depth=1)
        check(len(parents) >= 1,
              f"get_inheritance('FirmwareUpdate', up) → {len(parents)} parents")

        # 多层
        deep = q.get_inheritance("DeviceAdapter", direction="down", depth=-1)
...
def test_call_query():
    ...
    with CallQuery(DB_PATH) as cq:
        # get_callees
        callees = cq.get_callees("PerformUpdate")
        check(len(callees) >= 1,
              f"get_callees('PerformUpdate') → {len(callees)} callees")

        # get_callers — 找一个有调用方的函数
        callers = cq.get_callers("PerformUpdate")
        check(isinstance(callers, list),
              f"get_callers('PerformUpdate') → {len(callers)} callers")

        # call_chain
        chain = cq.get_call_chain("PerformUpdate", direction="down", depth=2)
        check(isinstance(chain, list),
              f"get_call_chain('PerformUpdate', down, depth=2) → {len(chain)} nodes")
...
def test_traverse():
    ...
    with TraverseQuery(DB_PATH) as tq:
        ...
        result = tq.traverse_graph(
            "PerformUpdate", direction="outgoing", depth=2, mode="bfs"
        )
        ...
        result_dfs = tq.traverse_graph(
            "PerformUpdate", direction="outgoing", depth=2, mode="dfs"
        )
...
def test_blast_radius():
    ...
    with BlastRadiusQuery(DB_PATH) as bq:
        result = bq.compute(symbols=["PerformUpdate"], depth=2)
        check(isinstance(result.affected_nodes, list),
              f"blast_radius('PerformUpdate') → {len(result.affected_nodes)} affected nodes")
...
def test_n1_query_correctness():
    ...
    with CallQuery(DB_PATH) as cq:
        ...
        if callers:
            ...
            check(c.callee_name == "PerformUpdate", f"callee_name correct: '{c.callee_name}'")
        ...
        if callees:
            c = callees[0]
            check(c.caller_name == "PerformUpdate", f"caller_name correct: '{c.caller_name}'")
...
def test_polymorphism():
    ...
    with PolymorphismQuery(DB_PATH) as pq:
        ...
        vfuncs = pq.get_virtual_functions("DeviceAdapter")
        check(len(vfuncs) >= 1,
              f"get_virtual_functions('DeviceAdapter') → {len(vfuncs)} virtual funcs")
        ...
        impls = pq.get_all_implementations("DeviceAdapter")
        check(isinstance(impls, list),
              f"get_all_implementations('DeviceAdapter') → {len(impls)} implementations")
        if impls:
            impl = impls[0]
