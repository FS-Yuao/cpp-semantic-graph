...
class AccuracyValidator:
    ...
    def _normalize_sig(sig: str) -> str:
        ...
        clangd: 'bool FirmwareUpdate::PerformUpdate()'  /  'public: bool FirmwareUpdate::PerformUpdate()'
        图谱:   'bool PerformUpdate() override'   /  'bool PerformUpdate() = 0'
        归一化: 去类名前缀、去 override/=0/修饰符，保留 '返回类型 函数名(参数)'
        """
        if not sig:
            return ""
        s = sig.replace("public:", "").replace("private:", "").replace("protected:", "")
        s = re.sub(r"\b\w+::", "", s)              # 去类名前缀 FirmwareUpdate::
    ...
    def _norm_file(path: str) -> str:
        ...
        图谱存相对路径如 'device_update/soc/chip_update.cpp'
        baseline 存 'example_update_service/src/device_update/soc/chip_update.cpp'
        归一化为最后两段路径，保证可比。
        """
        if not path:
            return ""
        p = path.replace("\\", "/")
        # 用文件名 + 父目录作为归一化键（足以区分 soc/sensor/switch/mcu）
