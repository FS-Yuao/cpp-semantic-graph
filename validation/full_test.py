...
def run_functional_tests(db_path: str) -> list[ToolCheck]:
    ...
    ok = "DeviceAdapter" in names
    checks.append(ToolCheck(
        "search_class", "DeviceAdapter", len(results), ms, ok,
        f"找到: {sorted(names)}"))
    ...
    with GraphQuery(db_path) as q:
        results, ms = timed(lambda: q.get_file_symbols("chip_update.h"))
    types = {s.node_type for s in results}
    ok = len(results) >= 1 and "class" in types
    checks.append(ToolCheck(
        "get_file_symbols", "chip_update.h", len(results), ms, ok,
        f"类型: {sorted(types)}"))
