...
REPO_SRC = Path("<PROJECT_SRC>/src")
REPO_INC = Path("<PROJECT_SRC>/include")
...
def run_functional_tests(db_path: str) -> list[ToolCheck]:
    ...
    with GraphQuery(db_path) as q:
        results, ms = timed(lambda: q.search_class("DeviceAdapter"))
    names = {c.name for c in results}
    expected_sub = {"FirmwareUpdate", "SensorUpdate", "SwitchUpdate", "McuUpdate"}
    ok = "DeviceAdapter" in names and expected_sub.issubset(names)
    checks.append(ToolCheck(
        "search_class", "DeviceAdapter (模糊)", len(results), ms, ok,
        f"找到: {sorted(names)}"))

    # 2. search_function
    with GraphQuery(db_path) as q:
        results, ms = timed(lambda: q.search_function("PerformUpdate"))
    classes_with_perform = {f.class_name for f in results if f.name == "PerformUpdate"}
    ok = len(results) >= 5 and "DeviceAdapter" in classes_with_perform
    checks.append(ToolCheck(
        "search_function", "PerformUpdate", len(results), ms, ok,
        f"类: {sorted(classes_with_perform)}"))

    # 3. get_inheritance
    with GraphQuery(db_path) as q:
        results, ms = timed(lambda: q.get_inheritance("DeviceAdapter", direction="down", depth=1))
    child_names = {i.child.name for i in results}
    ok = child_names == {"FirmwareUpdate", "SensorUpdate", "SwitchUpdate", "McuUpdate"}
    checks.append(ToolCheck(
        "get_inheritance", "DeviceAdapter down depth=1", len(results), ms, ok,
        f"子类: {sorted(child_names)}"))

    # 4. get_callers
    with CallQuery(db_path) as cq:
        results, ms = timed(lambda: cq.get_callers("PerformUpdate"))
    caller_files = {c.caller_file for c in results}
    ok = len(results) >= 2
    checks.append(ToolCheck(
        "get_callers", "PerformUpdate", len(results), ms, ok,
        f"调用方文件: {sorted(caller_files)}"))

    # 5. get_callees
    with CallQuery(db_path) as cq:
        results, ms = timed(lambda: cq.get_callees("PerformUpdate", class_name="FirmwareUpdate"))
    callee_names = {c.callee_name for c in results}
    ok = len(results) >= 1
    checks.append(ToolCheck(
        "get_callees", "PerformUpdate (FirmwareUpdate)", len(results), ms, ok,
        f"被调用: {sorted(callee_names)[:10]}"))

    # 6. get_overrides
    with PolymorphismQuery(db_path) as pq:
        results, ms = timed(lambda: pq.get_all_overrides("PerformUpdate", class_name="DeviceAdapter"))
    override_classes = {o.class_name for o in results}
    ok = override_classes == {"FirmwareUpdate", "SensorUpdate", "SwitchUpdate", "McuUpdate"}
    checks.append(ToolCheck(
        "get_overrides", "PerformUpdate (DeviceAdapter)", len(results), ms, ok,
        f"重写类: {sorted(override_classes)}"))

    # 7. get_file_symbols
    with GraphQuery(db_path) as q:
        results, ms = timed(lambda: q.get_file_symbols("chip_update.cpp"))
    types = {s.node_type for s in results}
    ok = len(results) > 5 and "class" in types and "function" in types
    checks.append(ToolCheck(
        "get_file_symbols", "chip_update.cpp", len(results), ms, ok,
        f"类型: {sorted(types)}"))

    # 8. traverse_graph
    with TraverseQuery(db_path) as tq:
        results, ms = timed(lambda: tq.traverse_graph("FirmwareUpdate", depth=2, max_results=50))
    ok = len(results.nodes) > 5
    checks.append(ToolCheck(
        "traverse_graph", "FirmwareUpdate depth=2", len(results.nodes), ms, ok,
...
def run_performance_benchmark(db_path: str) -> list[BenchResult]:
    ...
    # --- S1: 查类定义在哪 ---
    with GraphQuery(db_path) as q:
        graph_ms = bench(lambda: q.search_class("FirmwareUpdate"))
        graph_count = len(q.search_class("FirmwareUpdate"))

    grep_cmd = f"find {inc_dir} -name '*.h' -exec grep -l 'class FirmwareUpdate' {{}} +"
    ...
    # --- S2: 查谁继承了这个类 ---
    with GraphQuery(db_path) as q:
        graph_ms = bench(lambda: q.get_inheritance("DeviceAdapter", direction="down", depth=1))
        graph_count = len(q.get_inheritance("DeviceAdapter", direction="down", depth=1))

    grep_cmd = f"find {inc_dir} -name '*.h' -exec grep -l ': public DeviceAdapter' {{}} +"
    grep_ms, stdout = bench_subprocess(grep_cmd)
    grep_count = len([l for l in stdout.strip().splitlines() if l])
    speedup = grep_ms / graph_ms if graph_ms > 0 else float('inf')
    ...
    # --- S3: 查函数被谁调用 ---
    with CallQuery(db_path) as cq:
        graph_ms = bench(lambda: cq.get_callers("PerformUpdate"))
        graph_count = len(cq.get_callers("PerformUpdate"))

    grep_cmd = f"find {src_dir} -name '*.cpp' -exec grep -l 'PerformUpdate' {{}} +"
    grep_ms, stdout = bench_subprocess(grep_cmd)
    grep_count = len([l for l in stdout.strip().splitlines() if l])
    speedup = grep_ms / graph_ms if graph_ms > 0 else float('inf')
    ...
    results.append(BenchResult("S3:查调用方", graph_ms, grep_ms,
                               graph_count, grep_count, speedup,
                               "grep 返回含 PerformUpdate 的文件（含声明/定义/调用，无法区分）"))

    # --- S4: 查虚函数所有重写 ---
    with PolymorphismQuery(db_path) as pq:
        graph_ms = bench(lambda: pq.get_all_overrides("PerformUpdate", class_name="DeviceAdapter"))
        graph_count = len(pq.get_all_overrides("PerformUpdate", class_name="DeviceAdapter"))

    grep_cmd = (f"find {src_dir} {inc_dir} \\( -name '*.cpp' -o -name '*.h' \\) "
                f"-exec grep -l 'PerformUpdate.*override' {{}} +")
    grep_ms, stdout = bench_subprocess(grep_cmd)
    grep_count = len([l for l in stdout.strip().splitlines() if l])
    speedup = grep_ms / graph_ms if graph_ms > 0 else float('inf')
    ...
    # --- S5: 查文件内所有符号 ---
    with GraphQuery(db_path) as q:
        graph_ms = bench(lambda: q.get_file_symbols("chip_update.cpp"))
        graph_count = len(q.get_file_symbols("chip_update.cpp"))

    # grep 近似：找函数定义（粗略匹配）
    chip_file = f"{src_dir}/device_update/soc/chip_update.cpp"
    if Path(chip_file).exists():
        grep_cmd = f"grep -cE '^[a-zA-Z].*::.*\\(|^[a-zA-Z].* \\w+\\(' {chip_file}"
        grep_ms, stdout = bench_subprocess(grep_cmd)
        try:
    ...
    # --- S6: 多跳影响面分析 ---
    with TraverseQuery(db_path) as tq:
        graph_ms = bench(lambda: tq.traverse_graph("FirmwareUpdate", depth=2, max_results=50))
        r = tq.traverse_graph("FirmwareUpdate", depth=2, max_results=50)
        graph_count = len(r.nodes)

    # grep 等价：需多轮手动串联（第1轮找 FirmwareUpdate 的直接关联，第2轮找关联的关联...）
    # 这里模拟第1轮 grep
    grep_cmd = (f"find {src_dir} {inc_dir} \\( -name '*.cpp' -o -name '*.h' \\) "
                f"-exec grep -l 'FirmwareUpdate' {{}} +")
    grep_ms, stdout = bench_subprocess(grep_cmd)
    grep_count = len([l for l in stdout.strip().splitlines() if l])
    speedup = grep_ms / graph_ms if graph_ms > 0 else float('inf')
