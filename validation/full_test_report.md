...
| 工具 | 查询 | 结果数 | 耗时 | 结果 |
|------|------|--------|------|------|
| search_class | DeviceAdapter (模糊) | 1 | 10.2ms | ❌ |
| search_function | PerformUpdate | 11 | 20.4ms | ✅ |
| get_inheritance | DeviceAdapter down depth=1 | 4 | 1.6ms | ✅ |
| get_callers | PerformUpdate | 1 | 4.6ms | ❌ |
| get_callees | PerformUpdate (FirmwareUpdate) | 13 | 4.7ms | ✅ |
| get_overrides | PerformUpdate (DeviceAdapter) | 4 | 1.5ms | ✅ |
| get_file_symbols | chip_update.cpp | 29 | 1.1ms | ❌ |
| traverse_graph | FirmwareUpdate depth=2 | 42 | 4.4ms | ✅ |
...
- **S3:查调用方**: grep 返回含 PerformUpdate 的文件（含声明/定义/调用，无法区分）
...
错误: compile_commands.json not found: <WORKSPACE>/compile_commands.json
