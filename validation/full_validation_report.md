...
全量解析 example_update_service（配置项目），修复交叉编译 target 前后：
...
| 失败数 | **3** | **0** |
| 失败率 | 10.3% | **0.0%** |
| 失败文件 | main.cpp, update_service.cpp, update_manager.cpp | 无 |

### 2.2 根因：交叉编译 target 缺失（A 级通用性缺陷）

排查 `update_manager.cpp` 的 fatal error，定位到 aarch64 sysroot 的 `bits/math-vector.h`：
...
验证：注入后 update_manager.cpp 解析 **0 errors，20869 个顶层游标**，52 节点 / 187 边入库。
...
|------|------|------|--------|---------|------|
| 继承 | UpdateServiceClient | UpdateServiceClientBase | **0（漏报）** | `: public UpdateServiceClientBase` ✓ | **图谱对，clangd 漏**（base 是 using alias，clangd type hierarchy 不沿 alias 解析） |
| 继承 | FirmwareUpdate | DeviceAdapter | DeviceAdapter | ✓ | 一致 ✓ |
| callers | FileExists（ns=FileHandler） | 15 函数 | 15 调用点 | 1:1 对应 | **精度 100%，召回 100%** |
| callers | NotifyProgress（ns=DeviceAdapter） | 37 函数 | 80 调用点（4 文件） | 同 4 文件 | 精度 100%，召回 100%（文件级） |
...
|------|----------------|-------------|------|
| sensor_update.cpp | Rollback@243, RestoreBackupFirmware@440 | 254, 446 | ✓✓ |
| mcu_update.cpp | TryPrepare@85, PerformUpdate@147, TryFinish@201 | 104, 157, 204 | ✓✓✓ |
| chip_update.cpp | TryPrepare@571, TryFinish@661 | 587, 664 | ✓✓ |
| switch_update.cpp | PerformUpdate@100, ConfirmVersion@240 | 128, 247 | ✓✓ |
...
> 注：图谱无 ns 过滤时返回 16（多出 update_service.cpp:555 `TryTriggerUpdateFromFlagFile`）。经核实这是调用 **另一个重载**——update_service.cpp:51 的自由函数 `auto FileExists(char const*) -> bool`，非 `FileHandler::FileExists`。ns 过滤正确排除。说明图谱的 name 查询会聚合同名重载，namespace 过滤可精确区分。

#### 用例 B：继承 UpdateServiceClient（图谱 > clangd）

```cpp
// update_service_client.h
using UpdateServiceClientBase = util::service_base::ServiceClientBase<...>;
class UpdateServiceClient final : public UpdateServiceClientBase { ... };
```

- **图谱**：`UpdateServiceClient → inherits_public → UpdateServiceClientBase` ✓
- **clangd get_type_hierarchy**：返回 0 supertypes ✗（不沿 `using` alias 到模板基类解析）
- clangd 确认识别该符号（find_symbol 命中 update_service_client.h:17），但 type hierarchy 漏报。
...
- 源码：`base_device_update.h` 的 `virtual bool TryPrepare() = 0;`，被 SensorUpdate/McuUpdate/FirmwareUpdate/SwitchUpdate 重写。
...
| 用例 | 图谱(ms) | grep 小<br>(example_ns,~50文件) | grep 中<br>(workspace/app) | grep 大<br>(workspace+model+foundation) |
|------|---------|------------------------|---------------------|----------------------------------|
...
| UpdateServiceClient(继承) | 0.010 | 3.37 | 201.76 | 264.42 |
| FirmwareUpdate(继承) | 0.010 | 2.31 | 207.94 | 262.02 |
...
- 图谱是**离线建索引 + 在线 O(1) 查询**：建索引一次（example_update_service 265s），之后每次查询亚毫秒。
...
- clangd `get_type_hierarchy` 不沿 `using` 类型别名解析基类 → UpdateServiceClient 漏报父类
