...
跑核心模块（soc/sensor/switch/mcu update）的 AST 解析，生成中间 JSON，
...
# 核心模块筛选关键词（compile_commands 中 example_update_service 的 device_update + device_manager）
# device_manager 含 device_adapter.cpp（PerformUpdate 的调用方），验证调用关系维度需要
CORE_FILTERS = ["device_update/soc", "device_update/sensor",
                "device_update/switch", "device_update/mcu",
                "device_manager"]
...
def main():
    ...
    with GraphQuery(str(DB_PATH)) as q:
        ...
        classes, timings["search_class"] = timed(
            lambda: q.search_class("DeviceAdapter"), "search_class")
        print(f"\n  search_class('DeviceAdapter') [{timings['search_class']:.2f}ms]: "
              f"{len(classes)} 个")
        for c in classes:
        ...
        inh, timings["inheritance"] = timed(
            lambda: q.get_inheritance("DeviceAdapter", direction="down", depth=1),
            "inheritance")
        print(f"\n  get_inheritance('DeviceAdapter', down) "
        ...
        funcs, timings["search_function"] = timed(
            lambda: q.search_function("PerformUpdate"), "search_function")
        print(f"\n  search_function('PerformUpdate') "
        ...
        syms, timings["file_symbols"] = timed(
            lambda: q.get_file_symbols("chip_update.cpp"), "file_symbols")
        print(f"\n  get_file_symbols('chip_update.cpp') "
        ...
        expected = {"FirmwareUpdate", "SensorUpdate", "SwitchUpdate", "McuUpdate"}
        checks = [
            ("search_class 返回 DeviceAdapter",
             any(c.name == "DeviceAdapter" for c in classes)),
            ("inheritance down 返回 4 个子类", child_names == expected),
            ("search_function 返回 PerformUpdate",
             any(f.name == "PerformUpdate" for f in funcs)),
            ("file_symbols 返回 chip_update 符号", len(syms) > 0),
